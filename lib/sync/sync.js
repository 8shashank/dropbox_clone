...
function getDirectoryInfo(path, cb) {
    ...
    handler.list(path, function (files) {
        //Remove ignored files such that sync is unaware of files it's told to ignore
        // #Review#
        //      Change to live with Async
        removeIgnoredFiles(files,path,function(remainingFiles){
            cb({
...
// #Review#
// It might be helpful to move this variable declaration closer to where it's used.
var ignoredFilesContainer={};

// #Review#
// This might help to convert base64 to utf8
// Source http://stackoverflow.com/questions/26721893/convert-buffer-base64-utf8-encoding-node-js
var utf8encoded = function(base64string){
    return (new Buffer(base64string, 'base64')).toString('utf8');
};

function removeIgnoredFiles(files, path, cb) {

    var dropboxIgnore= "_.dropboxignore";

    // #Review#
    // Move the sentence here because of scope
    var ignoreFilePath = uris.getPath(path) + "/" + dropboxIgnore;

    // #Review#
    //      Use handlers defined by server and client
    var handler = getHandler(path); 

    // #Review
    // Wrap the check ignore file and update into functions
    var updateIgnoreFiles = function(stats){

        var mtime = stats.mtime;

        if ((!ignoredFilesContainer[path]) || mtime > ignoredFilesContainer[path].lastModifiedDate)
        {
            console.log("update ignore files needed");
            // #Review#
            // var data = fs.readFileSync(uris.getPath(path) + "/" + dropboxIgnore, {'encoding': 'utf8'});
            //      It might be better to use handlers and then convert to utf8

            handler.readFile(ignoreFilePath, function(string){
                
                var ignoreFileString = utf8encoded(string);

                // #Review#
                // var fileArray=data.split("\n");
                // var cleanedArray=[];
                // _.each(fileArray, function(file){
                //     cleanedArray.push(file.trim());
                // });
                //      JS map and apply function might help to simplify the sentences
                //      Source: http://stackoverflow.com/questions/19293997/javascript-apply-trim-function-to-each-string-in-an-array
                //      Also you might want to be compatible with windows \r\n

                var cleanedArray = ignoreFileString.replace("\r\n","\n").split("\n");
    ...
    var removeIgnoredFilesFromArray = function(){
        // #Review#
        /*
        _.each(ignoredFilesContainer[path].files, function (ignoredFile) {
            removeIgnoredFile(ignoredFile)
        });*/
        //          There are some short sentences to do this job like one below
        //          Source: https://lodash.com/docs

        console.log("Remove from array");
        console.log(ignoredFilesContainer[path].files);

        files = _.difference(files,ignoredFilesContainer[path].files);
        console.log(files);
    ...
    if (_.contains(files, dropboxIgnore)) {
        ...
        // #Review# 
        // var stats= fs.statSync(uris.getPath(path) + "/" + dropboxIgnore);
        //      As there are defined handlers for dnode/files,
        //      It might be consistent to use getHandlers rather than directly call fs.statSync

        handler.stat(ignoreFilePath,updateIgnoreFiles);
    }
    else{
        //If dropboxIgnore file has been deleted, clear cache

        // #Review
        // ignoredFilesContainer.files=[]
        //      This will remove ignoredFilesContainer["files"],
        //      But maybe you want to use the sentence below to clear cache for the given path?
        ignoredFilesContainer[path]=[];
        cb(files);
    }
}
