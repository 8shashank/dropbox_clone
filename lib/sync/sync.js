...
function compareDirectoriesOneWay(state1,state2,matcher, ignoredFiles){
	console.log("comparedironeway()");
	var directoriesMatch = true;

    var needsCopyingToOtherDirectory = [];

    for(var name in state1) {
        if (fileNotIgnored(name, ignoredFiles)) { //if false: logic skipped; file not queued in needsCopyingToOtherDirectory
...
function fileNotIgnored(name, ignoredFiles) {
	console.log("filenotignored()");
	console.log("filenotignored: " + name + ", " + ignoredFiles);
	//fixed here(youngho)
	/*simplified process using indexOf.
	  If the file exists in ignoredFiles, then indexOf returns > -1
	*/
...
function compare(path1,path2,matcher, ignoredFiles, cb){
    getDirectoryInfo(path1,function(directoryState1){
        getDirectoryInfo(path2,function(directoryState2){
            compareDirectories(directoryState1,directoryState2,matcher,cb,ignoredFiles);
        });
    });
}

function compareDirectories(directoryInfo1, directoryInfo2, matcher, cb, ignoredFiles){
    ...
    captureDirectoryState(directoryInfo1,function(state1){
        captureDirectoryState(directoryInfo2,function(state2){
            var syncTo2 =
                compareDirectoriesOneWay(state1,state2,matcher,ignoredFiles);

            var syncTo1 =
                compareDirectoriesOneWay(state2,state1,matcher,ignoredFiles);
