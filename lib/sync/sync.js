...
function getDirectoryInfo(path, cb) {
    ...
    handler.list(path, function (files) {
        ...
        remainingFiles = removeIgnoredFiles(files, path);
        cb({
...
function removeIgnoredFiles(files, path) {
    ...
    if (_.contains(files, dropboxIgnore)) {
        ...
        //Check if dropboxIgnore file has been updated for node at given path
        if (!ignoredFilesContainer[path] || modified > ignoredFilesContainer[path].lastModifiedDate){
        ...
        ignoredFilesContainer[path].files = [];
    }
