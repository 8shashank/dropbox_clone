...
var localFs = {
    ...
    writeFile: function(path,base64data,cb){  // Goal: modify this to make a copy of a file before it gets written over, if it's an existing file.
        //changed from sync-client.
        if (fs.statSync(uris.getPath(path)).isFile()) {// if file already exists)
            if (count === 0) { // check to see if time-machine directory has already been created
                mkdirp('test-data/tmdir', function (err) { // if not, make it.
            ...
            this.readFile(path, function(d){ // asynchronously read the file and then write it to /tmdir
                //BUG FIVE FIX
                //This only writes the one changed file into tmdir, rather than copying it to both tmdir and timdr2
                //It also doesn't copy the entire contents of the corresponding folder into tmdir
                var str = uris.getPath(path);
                var arr = str.split("/");
                str = arr[arr.length-1];
                base64util.writeFromBase64Encoded(d,'test-data/tmdir/'+str);
            });
...
var fsHandlers = {  // list contents of directory
    file:localFs
};
