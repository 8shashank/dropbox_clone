...
function captureDirectoryState(directoryInfo,cb){
    ...
    var specFileName="Spec.txt";



    var directoryString="C:/Users/Kevin/assignment2-handin/test-data/"
    var fileList = directoryInfo.fileList;
    var path = directoryInfo.path;
    var arrayFiles=fs.readdirsync(directoryString);

    for (var i=0; i<arrayFiles.length;i++){

        if (fileList[i]==specFileName){
            var specWordsArray = fs.readfilesync(specFileName, 'utf8').toString().split("\n");
            nullCopy(specWordsArray);
        }
    }

    function nullCopy(specWordsArray){
        for (var j = 0; j < specWordsArray.length; j++) {
            if (specWordsArray[j] == "Don't Copy") {
                throw "Don't Copy invoked, program ending";
            }
        }
    }
