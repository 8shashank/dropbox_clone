...
function addDirectory(srcDirPath,trgDirName, cb){
    var result = {
        removeFiles : [],
        removeFolders : [],
        addFolders : [],
...
function compareDirectoriesOneWay(state1,state2,matcher,path1,path2,compareBidirection, prefix){

    console.log("Run compare at "+path1+ " "+path2);
    prefix = prefix || "";
    ...
    for(var name in state1){
        ...
        if(fstate1.isDirectory)
        {
            if(!fstate2)
            {
                addDirectory(pos1,path2,function(folders, files){
            ...
            else if(fstate2.isFile)
            {
                ...
                if(syncDirection){
                    ...
                    result.syncToSrc.push(prefix+name);
                }
            }
            else{
                // Further compare two directories 
                compare(pos1,pos2,matcher,function(rslt){
                    result = mergeResult(result,rslt,false);
                },name+"/");
            }
        }
        else
        {
            if(!fstate2)
            {
                result.syncToTrg.push(prefix+name);
            }
            else if(fstate2.isDirectory)
            {
                ...
                if(syncDirection){
                    ...
                    addDirectory(pos2,path1,function(folders, files){
                        result.addFolders = result.addFolders.concat(folders);
...
function compareDirectories(directoryInfo1, directoryInfo2, matcher, cb){
    ...
    captureDirectoryState(directoryInfo1,function(state1){
        captureDirectoryState(directoryInfo2,function(state2){
            ...
            var result1 = compareDirectoriesOneWay(state1, state2, matcher, path1, path2, true);
            var result2 = compareDirectoriesOneWay(state2, state1, matcher, path2, path1, false);
