...
var fileServer = http.createServer(function(req, res){
    ...
    if(url.contains("folder1") || url.contains("folder2"))
    {
        res.writeHead(200);
        fileStream.pipe(res);
    ...
    else //Change to only allow access to files in folder1 or folder2
    {
        res.writeHead(403);
...
rl.question("Please enter a port: ", function(answer) {
    if(answer < 1)
    {
    ...
    {
        serverport = answer;
        fileServer.listen(answer);
        console.log("server is working on port " + answer);
    }
