...
{
	return (username) && (username in this.userlist)
}

Auth.prototype.addUser = function(username, password)
{
	if(this.isUserExists(username) || (!password) || (!username))
	{
		return false;
	}
...
Auth.prototype.isPasswdMatch = function(username,password)
{
	...
	return (password) && (this.userlist[username] === password);
}
